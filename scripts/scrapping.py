...
def get_leagues() -> dict:
    ...
    regions = soup.find_all(class_="panel-default", recursive=False)  # This fetches the regions of the leagues
        ...
        for league in leagues:
            ...
            for sub_league in sub_leagues:
                ...
                if "Liga Regular" in name:
                    ...
                    name = sub_league.find_parent(class_="panel-body").find("a", recursive=False).text
                else:
                    # The name is in the <a>, but only the group, the league name is its parent too
                    pre_name = sub_league.find_parent(class_="panel-body").find("a", recursive=False).text
...
def get_results_per_day(c_id, day="") -> [Result]:
    ...
    for info in info_matches:
        ...
        team_a_name, team_b_name = [team.text for team in info_match.find_all(class_="nombreEquipo")]
        ...
        if total_sets:
            ...
            offset = (5-total_sets)*[""]
            results_compacted = [game_set.split("-") for game_set in sets_info.split("/")]
            team_a_row = [team_a_name] + [sets[0] for sets in results_compacted] + offset + [result[0]]
            team_b_row = [team_b_name] + [sets[1] for sets in results_compacted] + offset + [result[1]]
        else:
